...
const Landing = () => {
    ...
    return (
        <div className="min-h-screen flex flex-col bg-[#FDFBF7] text-deep-black font-sans overflow-x-hidden selection:bg-note-purple/20">

            {/* --- DECORATIVE BACKGROUND ELEMENTS (Paper/Book vibes) --- */}
            {/* Top Right Purple Book Spine */}
            <div className={`absolute top-0 ${isRTL ? 'left-0' : 'right-0'} w-32 h-64 bg-note-purple rounded-b-lg shadow-2xl opacity-90 z-0 hidden md:block`}>
            ...
            <header className="sticky top-0 z-50 bg-[#FDFBF7]/90 backdrop-blur-sm">
                <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
                    ...
                    <div className="flex items-center gap-2 z-10">
                        <div className="w-12 h-12 bg-transparent border-2 border-note-purple text-note-purple rounded-full flex items-center justify-center font-bold text-lg shadow-sm">
                            Note
                        </div>
                    </div>
                    ...
                    <nav className="hidden md:flex gap-10 text-base font-medium text-gray-600 z-10">
                        <a href="#" className="hover:text-note-purple transition-colors relative group">
                            {t('menu_home')}
                            <span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-note-purple transition-all group-hover:w-full"></span>
                        </a>
                        <a href="#" className="hover:text-note-purple transition-colors relative group">
                            {t('menu_books')}
                            <span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-note-purple transition-all group-hover:w-full"></span>
                    ...
                    <div className="flex items-center gap-4 z-10">
                        ...
                        <button
                            ...
                            className="bg-deep-black text-white px-6 py-2 rounded-md font-bold hover:bg-gray-800 transition-all text-sm shadow-lg hover:shadow-xl"
                        >
                            {t('login_btn')}
                        </button>
                    </div>
            ...
            <section className="flex-1 flex flex-col justify-center items-center text-center px-6 py-12 relative z-10">
                ...
                <div className="max-w-4xl mx-auto relative">
                    <h1 className="text-6xl md:text-8xl font-black text-deep-black mb-4 tracking-tight leading-none font-cairo">
                ...
                <div className="flex flex-col md:flex-row gap-6 w-full max-w-md justify-center">
                    <button
                        onClick={() => navigate('/connexion')}
                        className="bg-deep-black text-white px-8 py-4 rounded-lg text-lg font-bold shadow-lg hover:transform hover:-translate-y-1 transition-all flex items-center justify-center gap-3"
                    >
                        {t('login_btn')}
                    </button>
                    <button
                        onClick={() => navigate('/connexion')}
                        className="bg-note-purple text-white px-8 py-4 rounded-lg text-lg font-bold shadow-lg hover:bg-opacity-90 hover:transform hover:-translate-y-1 transition-all flex items-center justify-center gap-3"
                    >
                ...
                {/* Bottom Deco */}
                <div className="mt-20 w-full max-w-5xl h-2 bg-gradient-to-r from-transparent via-note-purple to-transparent opacity-20"></div>
            ...
            <section className="py-20 px-6 max-w-7xl mx-auto grid md:grid-cols-2 gap-12 items-center relative z-10">
                ...
                <div className={`${isRTL ? 'order-1 md:order-1' : 'order-1 md:order-1'}`}>
                    <div className="border-l-4 border-note-purple pl-6 py-2">
                ...
                <div className="relative h-80 w-full bg-white shadow-2xl border-8 border-[#D4AF37]/20 p-8 flex items-center justify-center rotate-2 hover:rotate-0 transition-all duration-500">
                    <div className="absolute top-0 right-0 w-16 h-16 bg-note-purple clip-path-polygon"></div>
                    <BookOpen className="w-24 h-24 text-note-purple opacity-20" />
            ...
            <footer className="bg-white border-t border-gray-100 py-12 relative z-10">
                <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-6">
                    <div className="flex items-center gap-2">
                        <div className="w-8 h-8 bg-note-purple rounded-full"></div>
                        <span className="font-bold text-xl tracking-tighter">Note.dz</span>
                    </div>

                    <p className="text-gray-500 text-sm">{t('footer_text')}</p>

                    <a href="mailto:contact@notedz.com" className="text-note-purple font-medium hover:underline">
                        {t('contact_us')}
                    </a>
                </div>
                {/* Bottom bar decor */}
                <div className="h-2 w-32 bg-note-purple absolute bottom-0 left-10"></div>
